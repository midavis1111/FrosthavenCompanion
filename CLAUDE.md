...
- **`cards.json` + `perks.json`** (Domain, `EmbeddedResource`) + **`wwwroot/icons/cards/{classSlug}/{cardSlug}.png`** (App) — per-class ability cards (`CardCatalog`: `{handSize, cards:[{id,name,level,initiative,image}]}`) and perks/masteries (`PerkCatalog`: `{perks:[{text,boxes}], masteries:[]}`). **Authored classes: Blinkblade, Deathwalker, Geminate, Trapper.** Card art from `any2cards/worldhaven` `images/character-ability-cards/frosthaven/{abbrev}/` (bb/dw/ge/ta). Generated from `Lurkars/gloomhavensecretariat` `data/fh/character/{slug}.json` (perks) + `deck/{slug}.json` (cards) + `label/en.json` (`custom.fh.{slug}.N` text). **Trapper (GHS slug `trap`) has no `custom.fh.trap` labels upstream** → its custom perks/masteries are placeholders needing verification. The `/classes` page is a read-only gallery of all this. See project memory `characters-perks`.
